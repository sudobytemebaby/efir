...
type Config struct {
	...
	Room struct {
		Addr        string          `yaml:"addr"         env:"ROOM_SERVICE_ADDR"`
		CallTimeout time.Duration   `yaml:"call_timeout" env:"ROOM_CALL_TIMEOUT"`
		RetryDelays []time.Duration `yaml:"retry_delays" env:"RETRY_DELAYS"`
	} `yaml:"room"`
}
