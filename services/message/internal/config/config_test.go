...
func TestLoad(t *testing.T) {
	env := map[string]string{
		...
		"ROOM_SERVICE_ADDR":        "localhost:50053",
		"ROOM_CALL_TIMEOUT":        "5s",
	...
	tmp, err := os.CreateTemp("", "message_config_*.yaml")
	...
	assert.Equal(t, "localhost:50053", cfg.Room.Addr)
	assert.Equal(t, 5*time.Second, cfg.Room.CallTimeout)
}
...
func TestLoad_Defaults(t *testing.T) {
	...
	tmp, err := os.CreateTemp("", "message_defaults_*.yaml")
	require.NoError(t, err)
	defer func() { _ = os.Remove(tmp.Name()) }()
	_, err = tmp.WriteString("env: development\n")
	require.NoError(t, err)
...
func TestLoad_InvalidYAML(t *testing.T) {
	tmp, err := os.CreateTemp("", "message_invalid_*.yaml")
	require.NoError(t, err)
	defer func() { _ = os.Remove(tmp.Name()) }()
...
func TestLoad_InvalidEnvironment(t *testing.T) {
	...
	tmp, err := os.CreateTemp("", "message_env_*.yaml")
	require.NoError(t, err)
	defer func() { _ = os.Remove(tmp.Name()) }()
