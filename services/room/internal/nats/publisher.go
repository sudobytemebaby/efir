...
func (p *publisher) PublishRoomCreated(ctx context.Context, roomID uuid.UUID, recipientIDs []uuid.UUID) error {
	...
	_, err = p.js.Publish(ctx, SubjectRoomCreated, data)
	if err != nil {
...
func (p *publisher) PublishRoomDeleted(ctx context.Context, roomID uuid.UUID, recipientIDs []uuid.UUID) error {
	recipientIDStrs := make([]string, len(recipientIDs))
	for i, id := range recipientIDs {
		recipientIDStrs[i] = id.String()
	}
	...
	_, err = p.js.Publish(ctx, SubjectRoomDeleted, data)
	if err != nil {
		return fmt.Errorf("publish to nats: %w", err)
	}
